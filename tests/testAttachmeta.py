...
class testAttachmetaMethods(unittest.TestCase):
    ...
    def test_getSongName(self):
        self.assertEqual(attachmeta.getSongName('Source Code ft. Developers'), 'Source Code') 

    def test_getArtists(self):
        self.assertEqual(attachmeta.getArtists('artist1 & artist2'), ['artist1', 'artist2'] )
        self.assertEqual(attachmeta.getArtists('artist1 & artist2 ft. artist3', featuring = 1), ['artist1', 'artist2', 'artist3'])
        self.assertIsNone(attachmeta.getArtists('artist1 & artist2', featuring=1))

    def test_getData(self):
        self.assertEqual(attachmeta.getData('see you again', ['wiz khalifa'])["Track Name"], 'See You Again (feat. Charlie Puth)')
        self.assertIsNone(attachmeta.getData('some random song', ['artist1', 'artist2']))

    def test_stripNumbersAtBeginning(self):
        self.assertEqual(attachmeta.stripNumbersAtBeginning('12. song name'), 'song name')

    def test_getTheSong(self):
        data = attachmeta.getTheSong('see you again ft. Charlie Puth')
        self.assertEqual(data["Track Name"], 'See You Again (feat. Charlie Puth)')
